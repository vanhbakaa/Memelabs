...
class Tapper:
    ...
    async def buy_random_memes(self, http_client: aiohttp.ClientSession, memes):
        ...
        try:
            ...
            if res.status == 201:
                data = await res.json()
                logger.success(
                    f"{self.session_name} | <green>Successfully bought <yellow>{meme_to_buy['key']}</yellow> meme!</green>")
                return data
            else:
                return None
        except:
    ...
    async def upgrade(self, card, http_client: aiohttp.ClientSession):
        try:
            ...
            if res.status == 201:
                data = await res.json()
                logger.success(
                    f"{self.session_name} | <green>Successfully upgraded: <cyan>{card['name']}</cyan> to lvl: <red>{data['userLevel']}</red></green>")
                return True
            else:
                return None
        except:
    ...
    async def claim_cipher(self, text, http_client: aiohttp.ClientSession):
        try:
            ...
            if res.status == 201:
                data = await res.json()
                logger.success(
                    f"{self.session_name} | <green>Successfully claimed daily cipher, reward: <cyan>{data['rewards']}</cyan></green>")
                return True
            else:
                return None
        except:
    ...
    async def main_task(self, http_client: aiohttp.ClientSession, stop_event):
        try:
            ...
            if check is None:
                logger.warning(
                    f"{self.session_name} | <yellow>Something went wrong while getting ref info, aborting...</yellow>")
                stop_event.set()
                return
            ...
            if bonus_info is None:
                logger.warning(
                    f"{self.session_name} | <yellow>Something went wrong while getting bonus info, aborting...</yellow>")
                stop_event.set()
                return
            ...
            if bonus_info['mine']['accumulatedMinedReward'] > 0:
                ...
                if a:
                    logger.success(
                        f"{self.session_name} | <green>Successfully claimed <cyan>{bonus_info['mine']['accumulatedMinedReward']}</cyan> from mine!</green>")
                bonus_info = await self.get_bonus(http_client)
                self.balance = bonus_info['balance']
                logger.info(f"{self.session_name} | Balance: <cyan>{self.balance}</cyan>")
            ...
            if settings.AUTO_CIPHER:
                ...
                if timestamp > now:
                    ...
                    cipher = requests.get(
                        "https://raw.githubusercontent.com/vanhbakaa/nothing/refs/heads/main/memelabs")
            ...
            if settings.AUTO_TASK:
                ...
                for taskGroup in tasks:
                    ...
                    for task in taskInGroupList:
                        if task['type'] == "WalletExchangeTask" or task['type'] == "TelegramChannelTask" or task[
                            'type'] == "ReferralTask" or task['type'] == "TelegramGroupTask":
                            continue
                        if task['isActive']:
                            if task['isCompleted'] is False:
                                ...
                                if do_task is None:
                                    logger.warning(
                                        f"{self.session_name} | <yellow>Failed to start task: <red>{task['name']}</red></yellow>")
                                    continue
                                ...
                                if claim_taskss is None:
                                    logger.warning(
                                        f"{self.session_name} | <yellow>Failed to claim task: <red>{task['name']}</red></yellow>")
                                    continue
                                await asyncio.sleep(random.randint(2, 5))
...
async def run_query_tapper1(querys: list[str], proxies):
    ...
    while True:
        ...
        sleep_ = randint(settings.DELAY_EACH_ROUND[0], settings.DELAY_EACH_ROUND[1])
        logger.info(f"<red>Sleep {sleep_} seconds...</red>")
        await asyncio.sleep(sleep_)
